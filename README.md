...
| P 盤機 | 每個GPU需要30核 | 每個GPU需要 64 G | 需要 | 1TiB 用於緩存 plot 數據 | 萬兆網卡*2 | `plot-server` `sharded-cache` `full-piece-cache` |
...
command=/root/autonomys/autonomys-node run --base-path /var/autonomys-node --farmer --rpc-listen-on 0.0.0.0:9944 --chain mainnet --blocks-pruning=archive --state-pruning=archive --sync full --rpc-methods unsafe --rpc-cors all
autorestart=true
user=root
redirect_stderr=true
stdout_logfile_maxbytes=100MB
...
注意：這是node數據，还需要使用autonomys-export-piece工具把其轉換成piece資料才能用於封裝。
